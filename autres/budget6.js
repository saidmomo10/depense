...
const toStorage = (what,{budget,nom_depense,prix_depense,old_name}) => {
    if(typeof Storage !== undefined){
        ...
        if(user6 === null){localStorage.setItem("user6",JSON.stringify({budget:0,expenses:[],modify:new Date().toLocaleString()}))}
        user6 = localStorage.getItem("user6");
        user6 = JSON.parse(user6);
        if(what === "updateBudget"){
            localStorage.setItem("user6",JSON.stringify({...user6,budget,modify:new Date().toLocaleString()}));
        }else if(what === "addExpense"){
            let isExists = user6.expenses.some((obj) => obj.name === nom_depense );
            if(isExists){
                alert(nom_depense+" already exists");
            }else{
                user6.expenses.push({name:nom_depense,amt:prix_depense});
                localStorage.setItem("user6",JSON.stringify({...user6,modify:new Date().toLocaleString()}));
            }
        }else if(what === "updateExpense"){
            user6.expenses = user6.expenses.map((val) => {
                if(val.name === old_name){ return {name:nom_depense,amt:prix_depense} };
                return val;
            });
            localStorage.setItem("user6",JSON.stringify({...user6,modify:new Date().toLocaleString()}));
        }else if(what === "load"){
            modify_elem.innerText = "Dernière mise à jour effectuée le "+user6.modify;
            bud.innerText = user6.budget+"Fcfa";
            let total_bal = user6.budget;
            let exp_amount = 0;
            if(user6.expenses.length > 0){
                table.classList.remove("d-none");
                tbody.innerHTML = "";
                let position = 1;
                user6.expenses.forEach((val) => {
                    ...
                    total_bal -= val.amt;
                    localStorage.setItem("user6",JSON.stringify({...user6,total_bal,exp_amount}));
            ...
            user6.expenses = user6.expenses.filter((val) => {
