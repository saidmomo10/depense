...
const toStorage = (what,{budget,nom_depense,prix_depense,old_name}) => {
    if(typeof Storage !== undefined){
        ...
        if(user4 === null){localStorage.setItem("user4",JSON.stringify({budget:0,expenses:[],modify:new Date().toLocaleString()}))}
        user4 = localStorage.getItem("user4");
        user4 = JSON.parse(user4);
        if(what === "updateBudget"){
            localStorage.setItem("user4",JSON.stringify({...user4,budget,modify:new Date().toLocaleString()}));
        }else if(what === "addExpense"){
            let isExists = user4.expenses.some((obj) => obj.name === nom_depense );
            if(isExists){
                alert(nom_depense+" already exists");
            }else{
                user4.expenses.push({name:nom_depense,amt:prix_depense});
                localStorage.setItem("user4",JSON.stringify({...user4,modify:new Date().toLocaleString()}));
            }
        }else if(what === "updateExpense"){
            user4.expenses = user4.expenses.map((val) => {
                if(val.name === old_name){ return {name:nom_depense,amt:prix_depense} };
                return val;
            });
            localStorage.setItem("user4",JSON.stringify({...user4,modify:new Date().toLocaleString()}));
        }else if(what === "load"){
            modify_elem.innerText = "Dernière mise à jour effectuée le "+user4.modify;
            bud.innerText = user4.budget+"Fcfa";
            let total_bal = user4.budget;
            let exp_amount = 0;
            if(user4.expenses.length > 0){
                table.classList.remove("d-none");
                tbody.innerHTML = "";
                let position = 1;
                user4.expenses.forEach((val) => {
                    ...
                    total_bal -= val.amt;
                    localStorage.setItem("user4",JSON.stringify({...user4,total_bal,exp_amount}));
            ...
            user4.expenses = user4.expenses.filter((val) => {
