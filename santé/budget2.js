...
const toStorage = (what,{budget,nom_depense,prix_depense,old_name}) => {
    if(typeof Storage !== undefined){
        ...
        if(user2 === null){localStorage.setItem("user2",JSON.stringify({budget:0,expenses:[],modify:new Date().toLocaleString()}))}
        user2 = localStorage.getItem("user2");
        user2 = JSON.parse(user2);
        if(what === "updateBudget"){
            localStorage.setItem("user2",JSON.stringify({...user2,budget,modify:new Date().toLocaleString()}));
        }else if(what === "addExpense"){
            let isExists = user2.expenses.some((obj) => obj.name === nom_depense );
            if(isExists){
                alert(nom_depense+" already exists");
            }else{
                user2.expenses.push({name:nom_depense,amt:prix_depense});
                localStorage.setItem("user2",JSON.stringify({...user2,modify:new Date().toLocaleString()}));
            }
        }else if(what === "updateExpense"){
            user2.expenses = user2.expenses.map((val) => {
                if(val.name === old_name){ return {name:nom_depense,amt:prix_depense} };
                return val;
            });
            localStorage.setItem("user2",JSON.stringify({...user2,modify:new Date().toLocaleString()}));
        }else if(what === "load"){
            modify_elem.innerText = "Dernière mise à jour effectuée le "+user2.modify;
            bud.innerText = user2.budget+"Fcfa";
            let total_bal = user2.budget;
            let exp_amount = 0;
            if(user2.expenses.length > 0){
                table.classList.remove("d-none");
                tbody.innerHTML = "";
                let position = 1;
                user2.expenses.forEach((val) => {
                    ...
                    total_bal -= val.amt;
                    localStorage.setItem("user2",JSON.stringify({...user2,total_bal,exp_amount}));
            ...
            user2.expenses = user2.expenses.filter((val) => {
