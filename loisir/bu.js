...
const toStorage = (what,{budget,nom_depense,prix_depense,old_name}) => {
    if(typeof Storage !== undefined){
        let user5 = localStorage.getItem("user5");
        if(user5 === null){localStorage.setItem("user5",JSON.stringify({budget:0,expenses:[],modify:new Date().toLocaleString()}))}
        user5 = localStorage.getItem("user5");
        user5 = JSON.parse(user5);
        if(what === "updateBudget"){
            localStorage.setItem("user5",JSON.stringify({...user5,budget,modify:new Date().toLocaleString()}));
        }else if(what === "addExpense"){
            let isExists = user5.expenses.some((obj) => obj.name === nom_depense );
            if(isExists){
                alert(nom_depense+" already exists");
            }else{
                user5.expenses.push({name:nom_depense,amt:prix_depense});
                localStorage.setItem("user5",JSON.stringify({...user5,modify:new Date().toLocaleString()}));
            }
        }else if(what === "updateExpense"){
            user5.expenses = user5.expenses.map((val) => {
                if(val.name === old_name){ return {name:nom_depense,amt:prix_depense} };
                return val;
            });
            localStorage.setItem("user5",JSON.stringify({...user5,modify:new Date().toLocaleString()}));
        }else if(what === "load"){
            modify_elem.innerText = "Dernière mise à jour effectuée le "+user5.modify;
            bud.innerText = user5.budget+"Fcfa";
            let total_bal = user5.budget;
            let exp_amount = 0;
            if(user5.expenses.length > 0){
                table.classList.remove("d-none");
                tbody.innerHTML = "";
                user5.expenses.forEach((val) => {
                    tbody.insertAdjacentHTML("beforeend",`<tr><td>${val.name}</td><td>${val.amt}</td><td>
                    <button class='btn btn-green' type='button' onclick="updateExp('${val.name}','${val.amt}')"><i style="color: green;" class='fa-solid fa-pen-to-square'></i></button>
                    <button class='btn btn-red' type='button' onclick="deleteExp('${val.name}')"><i style="color: red;"  class='fa-solid fa-trash-can'></i></button>
                    </td></tr>`);
...
const barChart = new Chart(barCanvas, {
    ...
    option:{
        scale:{
            ...
            user5.expenses = user5.expenses.filter((val) => {
